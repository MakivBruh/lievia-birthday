import { motion } from "framer-motion";

export default function HeroSection() {
    return (
        <div className="relative h-170 w-full flex flex-col justify-center items-center text-center text-white select-none">
        
            {/* Background Image */}
            <div className="absolute inset-0">
                <img
                    src="/image/veronika.jpg" // ganti dengan gambar kamu
                    alt="blue flower background"
                    className="w-full h-full object-cover"
                />
                <div className="absolute inset-0 bg-black/40"></div>  
            </div>

            {/* Konten */}
            <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}className="relative z-10">
                <p className="text-lg mb-2">You're Invited!</p>
                <p className="text-sm mb-4">The Sweet Seventeen Birthday Party of</p>
                <h1 className="text-7xl md:text-6xl mt-10 font-bold font-kapakana">Lievia Veren</h1>
            </motion.div>

            
            <div className="absolute bottom-0 left-0 w-full  overflow-hidden leading-none pt-16">
                <motion.svg
                xmlns="http://www.w3.org/2000/svg"
                preserveAspectRatio="none"
                viewBox="0 0 1440 320"
                className="w-180 sm:w-250 md:w-300 lg:w-full h-30 sm:h-36 md:h-40 lg:h-48 text-sky-400 scale-125 drop-shadow-[0_0_20px_rgba(0,188,255,0.7)]"
                fill="currentColor"
                initial={{ x: 0 }}
                animate={{ x: ["0%", "10%", "0%"] }} // gerak kanan kiri
                transition={{
                duration: 8,
                repeat: Infinity,
                ease: "easeInOut",
                }}>
                    <path
                    fill="currentColor"
                    fillOpacity="1"
                    d="M0,160L48,133.3C96,107,192,53,288,32C384,11,480,21,576,53.3C672,85,768,139,864,144C960,149,1056,107,1152,96C1248,85,1344,107,1392,117.3L1440,128L1440,320L1392,320C1344,320,1248,320,1152,320C1056,320,960,320,864,320C768,320,672,320,576,320C480,320,384,320,288,320C192,320,96,320,48,320L0,320Z"></path>
                </motion.svg>
            </div>

            {/* Quote */}
            <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true }}
            className="absolute -bottom-8 w-full text-center py-4 z-20 ">
                <motion.svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 640 640"
                fill="white"
                className="drop-shadow-lg mx-auto w-15 sm:w-16 md:w-18 lg:w-20"
                initial={{ scale: 0.8, opacity: 0 }}
                whileInView={{ scale: 1, opacity: 1 }}
                transition={{ duration: 0.8 }}>
                <path d="M182.4 53.5L157.8 95.6C154 102.1 152 109.6 152 117.2L152 120C152 142.1 169.9 160 192 160C214.1 160 232 142.1 232 120L232 117.2C232 109.6 230 102.2 226.2 95.6L201.6 53.5C199.6 50.1 195.9 48 192 48C188.1 48 184.4 50.1 182.4 53.5zM310.4 53.5L285.8 95.6C282 102.1 280 109.6 280 117.2L280 120C280 142.1 297.9 160 320 160C342.1 160 360 142.1 360 120L360 117.2C360 109.6 358 102.2 354.2 95.6L329.6 53.5C327.6 50.1 323.9 48 320 48C316.1 48 312.4 50.1 310.4 53.5zM413.8 95.6C410 102.1 408 109.6 408 117.2L408 120C408 142.1 425.9 160 448 160C470.1 160 488 142.1 488 120L488 117.2C488 109.6 486 102.2 482.2 95.6L457.6 53.5C455.6 50.1 451.9 48 448 48C444.1 48 440.4 50.1 438.4 53.5L413.8 95.6zM224 224C224 206.3 209.7 192 192 192C174.3 192 160 206.3 160 224L160 277.5C122.7 290.6 96 326.2 96 368L96 388.8C116.9 390.1 137.6 396.1 156.3 406.8L163.4 410.9C189.7 425.9 222.3 424.3 247 406.7C290.7 375.5 349.3 375.5 393 406.7C417.6 424.3 450.3 426 476.6 410.9L483.7 406.8C502.4 396.1 523 390.1 544 388.8L544 368C544 326.2 517.3 290.6 480 277.5L480 224C480 206.3 465.7 192 448 192C430.3 192 416 206.3 416 224L416 272L352 272L352 224C352 206.3 337.7 192 320 192C302.3 192 288 206.3 288 224L288 272L224 272L224 224zM544 437C531.3 438.2 518.9 442 507.5 448.5L500.4 452.6C457.8 476.9 405 474.3 365.1 445.8C338.1 426.5 301.9 426.5 274.9 445.8C235 474.3 182.2 477 139.6 452.6L132.5 448.5C121.1 442 108.7 438.1 96 437L96 512C96 547.3 124.7 576 160 576L480 576C515.3 576 544 547.3 544 512L544 437z"/>
                </motion.svg>
                <blockquote className="px-4 mx-auto text-sm md:text-lg font-semibold drop-shadow-xl pt-1 italic text-white/90 max-w-5xl leading-relaxed drop-shadow-md">
                    Bersukacitalah dan bersyukurlah karena Tuhan. Bersukacitalah karena kasih setia-Nya dan bersyukurlah karena Dia mempertemukan kita semua di acara ulang tahun ini.
                </blockquote>
            </motion.div>
        </div>
    );
}